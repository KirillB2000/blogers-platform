...
const shortDescriptionValidation = body('shortDescription')
    ...
    .trim()
    .notEmpty().withMessage('Short description is required and cannot be empty')
    .isLength({max: 100}).withMessage('Short description is too long')

const content = body('content')
    .isString().withMessage('Content must be a string')
    .trim()
    .notEmpty().withMessage('Content is required and cannot be empty')
    .isLength({max: 1000}).withMessage('Content is too long')

const blogId = body('blogId')
    .isString().withMessage('Blog id must be a string')
    .trim()
    .notEmpty().withMessage('Blog id is required and cannot be empty')
    .custom(value => {
